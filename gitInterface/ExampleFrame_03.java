...
public class ExampleFrame_03 extends JFrame{
	...
	InternalFrame pInFrame = null;// 定义一个就绪内部窗体对象
	InternalFrame rInFrame = null;// 定义一个工作内部窗体对象
	InternalFrame tInFrame = null;// 定义一个休眠内部窗体对象
	InternalFrame qInFrame = null;// 定义一个侦听内部窗体对象
	InternalFrame sInFrame = null;// 定义一个灭活内部窗体对象
	...
	public ExampleFrame_03() {
		...
		ReadyButton.setText("就绪");
		ReadyButton
				.addActionListener(new BAListener(pInFrame, "就绪"));
		panel.add(ReadyButton);
		
		final JButton workButton = new JButton();
		workButton.setText("工作");
		workButton
				.addActionListener(new BAListener(rInFrame, "工作"));
		panel.add(workButton);
		
		final JButton sleepButton = new JButton();
		sleepButton.setText("休眠");
		sleepButton
				.addActionListener(new BAListener(tInFrame, "休眠"));
		panel.add(sleepButton);
		
		final JButton interceptButton = new JButton();
		interceptButton.setText("侦听");
		interceptButton
				.addActionListener(new BAListener(qInFrame, "侦听"));
		panel.add(interceptButton);
		
		final JButton InactivatedButton = new JButton();
		InactivatedButton.setText("灭活");
		InactivatedButton
				.addActionListener(new BAListener(sInFrame, "灭活"));
